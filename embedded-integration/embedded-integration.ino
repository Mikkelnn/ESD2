...
// #define SAMPEL_RATE 80000 // 80Khz
// #define DATA_BUFFER (SAMPEL_RATE / 5) // samples for 200ms
#define ADC_BIT_RESOLUTION 8
// #define N_CHANNELS 4
// #define PULSE_WIDTH_MS 1 // 20ms pulse length ~6.86m sound travel

AngleOfArrival* signalProcessing = new AngleOfArrival();
...
const int ADC_PWM_freq = SAMPEL_RATE;
...
TaskHandle_t xHandle_vTaskAcquireData, xHandle_vTaskPulseControl, xHandle_vTaskSignalProcessing;
...
void vTaskSignalProcessing(void *pvParameters) {
  ...
  for (;;) {
    ...
    int error = signalProcessing->calculateAngleAndDistance(((uint8_t (*)[N_CHANNELS])buffer), &angle, &distance_cm);
    if (error) {
      Serial.print("DSP error code: "); Serial.println(error, DEC);
    }
    
    Serial.print("Processing time (us): "); Serial.println(micros() - start);
    Serial.print("angle: "); Serial.print(angle); Serial.print(", distance (cm): "); Serial.println(distance_cm);
  }
}
...
void setup() {
  ...
  xTaskCreatePinnedToCore(vTaskSignalProcessing, "vTaskSignalProcessing", 1000, NULL, configMAX_PRIORITIES - 2, &xHandle_vTaskSignalProcessing, 0);
...
void loop() {
  ...
  if (input == 'r') {
    ...
    // Start the signal processing, this task will first start after "AcquireData" 
    // - as it is scheduled on the same core with lower priority
    vTaskResume(xHandle_vTaskSignalProcessing);
  } else if (input == 'd') {
    ...
    // load data into buffer and process it
    for (int i = 0; i < DATA_BUFFER; i++) {
      for (int j = 0; j < N_CHANNELS; j++) {
        while (!Serial.available());
        ((uint8_t (*)[N_CHANNELS])buffer)[i][j] = Serial.read();
      }
    }
  } else if (input == 'p') {
